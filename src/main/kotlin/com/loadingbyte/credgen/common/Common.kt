package com.loadingbyte.credgen.common
...
val LOGGER: Logger = LoggerFactory.getLogger("credgen")
...
val CONFIG_DIR: Path = when {
    SystemInfo.isWindows -> Path(System.getenv("APPDATA"), "credgen")
    SystemInfo.isMacOS -> Path(System.getProperty("user.home"), "Library", "Application Support", "credgen")
    else /* Linux & fallback */ -> {
        val cfgDir = System.getenv("XDG_CONFIG_HOME")
        if (cfgDir.isNullOrEmpty())
            Path(System.getProperty("user.home"), ".config", "credgen")
        else
            Path(cfgDir, "credgen")
    }
}
